...
from app.models import Attendance, Attendance_Report, Course,Session_Year,CustomUser, Staff,Student, Student_feedback, Student_leave, Subject,Staff_Notification,Staff_leave,Staff_feedback,Student_Notification
...
def ADD_STUDENT(request):
    ...
    if request.method=="POST":
        ...
        if CustomUser.objects.filter(username=username).exists():
            ...
            student.save()
            messages.success(request,user.first_name + " " + user.last_name + 'Profile Added Succesfully As a Student')
            return redirect('add_student')
...
def DELETE_STUDENT(request,admin):
    ...
    student.delete()
    messages.succes(request,'Record are Succesfully Deleted')
    return redirect('view_student')
...
def UPDATE_STAFF(request):
    if request.method =="POST":
        ...
        staff.save()
        messages.success(request,'Staff is Succesfully Updated')
...
def ADD_SUBJECT(request):
    ...
    if request.method =="POST":
        ...
        subject.save()
        messages.success(request,'Subjects are Added Succesfully')
...
def UPDATE_SESSION(request):
    if request.method == "POST":
        ...
        session.save()
        messages.success(request,'Session Updated Succesfully!!')
...
def STAFF_FEEDBACK_REPLY_SAVE(request):
    ...
    return redirect('staff__feedback')
